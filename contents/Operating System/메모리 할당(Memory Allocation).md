...
### 페이징 시스템
#### 주소 매핑 
- 페이지 테이블을 참조하여 논리 주소를 물리 주소로 매핑한다.


- 과정 
- 논리 주소를 페이지 번호(p)와 오프셋(d)으로 나눈다.
- 페이지 번호(p)를 페이지 테이블에 대한 인덱스로 사용하여 페이지 테이블의 p 위치에 저장된 페이지 프레임 번호(f)를 얻는다.
- 페이지 프레임 번호(f)에 페이지 오프셋(d)을 결합하여 실제 메모리 주소(물리 주소)를 얻는다.
> 오프셋(offset = 변위displacemnet)(d)는 페이지 시작부터 얼마나 떨어져있는가를 나타낸다.

#### 페이징 시스템의 논리 주소
- 페이지 크기는 2의 거듭제곱($2^n$)이며, 하드웨어에 의해 정의된다.
- 페이지 크기가 $2^n$이면, 논리 주소의 하위 n 비트는 페이지 오프셋을 나타내고, 나머지 상위 비트는 페이지 번호를 나타낸다.
...
- 페이지 테이블의 모든 항목을 메인 메모리에 유지하되, 일부 항목 정보를 빠른 연관 레지스터에 유지한다.
- 최근 참조한 일부 페이들에 대한 매핑 정보만 연관 레지스터에 유지한다.
- 프로세스가 최근 참조한 것을 또 참조하는 경향인 지역성을 기반으로 주소 변환을 한다.
- 적중률이 높을 수록 유효접근시간이 짧아진다.
- 연관 레지스터의 수가 증가하면 적중률이 상승한다.
> 적중률(hit ratio): 페이지 번호가 연관 레지스터에서 발견되는 비율  
> 연관 레지스터 탐색 시간: 50ns, 메모리 접근 시간 750ns  
> 적중하는 경우: 50 + 750 = 800ns, 실패한 경우: 50 + 750 +750 = 1550ns  
> 적중률 80%: 0.8 * 800 + 0.2 * 1550 = 950ns, 적중률 90%: 0.9 x 800 + 0.1 x 1550 = 875 ns
- 논리적 주소를 물리주소로 변환하는 과정)
  1. 연관 매핑 수행: 연관 레지스터에서 원하는 페이지의 매핑 정보 검색한다.
  2. 연관 레지스터에서 실패시 직접 매핑: 메인 메모리의 페이지 테이블에서 매핑 정보를 얻는다.
     - 참조한 매핑 정보를 연관 사상표(메모리)에 저장한다.
  3. 위에서 얻은 매핑 정보로 주소 변환
